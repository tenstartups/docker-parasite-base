...
class ParasiteConfig
  ...
  def build_environment_files
    ...
    %w(host container profile).each do |env_type|
      env_dir = File.join(ENV['PARASITE_CONFIG_DIRECTORY'], 'env')
      FileUtils.mkdir_p(env_dir)
      File.open(File.join(env_dir, "parasite-#{env_type}.env"), 'w') do |env_file|
        environment = {}
        ...
        environment['PARASITE_DOCKER_IMAGE_PARASITE_CONFIG'] = docker_image if env_type == 'host'
        if env_type == 'profile'
