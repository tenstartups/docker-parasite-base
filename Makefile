...
run: build
	docker run -it --rm -v /var/run/docker.sock:/var/run/docker.sock:ro -v $(PWD)/parasite-config:/parasite-config -e PARASITE_VAR=test --net host $(DOCKER_IMAGE_NAME) $(ARGS)
